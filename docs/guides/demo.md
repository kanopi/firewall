...
=== "`/` — allowed"
    ...
    ![The demo landing page, headed "You passed the firewall.", listing the four demo routes and their expected behavior](../assets/images/demo/allowed-response.png "The request reached the application because no plugin matched")

=== "`/admin` — blocked"

    ![A plain-text block response reading "Blocked: /admin is restricted (request 6236CCE83C3BB64804BFEF16CB8D64AC)."](../assets/images/demo/blocked-response.png "The demo's banning_message, with {{request.id}} interpolated")

    The message text comes from `global.banning_message` in
    `example/demo/config.yml`. The trailing identifier is the `{{request.id}}`
    placeholder — see [Banning Message](../configuration/global.md#banning-message)
    for the full placeholder list.

=== "`/secure` — challenged"

    ![The math challenge interstitial asking "What is 2 + 7?"](../assets/images/challenges/math-interstitial.png "The math provider's interstitial, served in place of the requested page")

=== "`/secure-altcha` — challenged"

    ![The ALTCHA challenge interstitial in its "Verifying..." state](../assets/images/challenges/altcha-interstitial.png "The ALTCHA widget solving its proof-of-work")

!!! tip "Blocks are sticky"

    Hitting `/admin` writes your IP to the demo's `FileStorage`, so the *next*
